export const aboutContent = {
  hero: {
    mission: "Aashray Infotech — Building a Safer Digital Planet",
    paragraphs: [
      "Aashray Infotech Private Limited is an Indian technology startup built around a shared mission: to build a digital planet that is safe by design—for enterprises and future generations.",
      "The digital world is evolving rapidly. With AI reshaping how we work and live, our responsibility is to make this digital landscape not just smarter, but safer and more human.",
      "We build AI-first solutions to tackle emerging challenges—from DPDP Act compliance to AI surveillance and digital mental well-being for students.",
      "Our intelligent agents automate repetitive tasks, simplify complex processes, and surface insights to help you interact with technology more effectively.",
      "As we grow, our objective remains consistent: use technology to make the complex simple, reduce mundane work, and strengthen digital safety."
    ]
  },
  values: {
    heading: "Everything we build is guided by three principles:",
    items: [
      { title: "Transparency", description: "Technology should be clear, understandable, and accountable—especially when it involves people, processes, and data.", icon: "users" },
      { title: "Innovation", description: "We continuously explore AI, agentic systems, and emerging technologies to solve real-world problems in new and meaningful ways, which you will also see in newer versions.", icon: "lightbulb" },
      { title: "Trust", description: "Trust begins with how we handle data. Your data stays with you. Our approach is built around keeping your data on your own infrastructure (on prem/cloud), giving you greater control over where your data resides and how it is managed.", icon: "shield" }
    ]
  },
  closing: {
    heading: "Our Aashray",
    paragraphs: [
      "We approach every problem with the same philosophy:",
      "Technology should make the digital world safer, smarter, and better for the people who use it.",
      "Aashray means protection and shelter. We aspire to build a digital aashray where people, organizations, and their data are fiercely protected in a connected world."
    ]
  },
  leadership: {
    heading: "Leadership Team",
    members: [
      {
        name: "Miss Ashima Girdhar",
        role: "CEO",
        image: "/about/Ashima_Girdhar.png"
      },
      {
        name: "Mr. Ashish Girdhar",
        role: "Chief Revenue Officer",
        image: "/about/Ashish_Girdhar.png"
      },
      {
        name: "Mrs. Mamta Girdhar",
        role: "Chief Legal and Human Resources",
        image: "/about/Mamta_Girdhar.png"
      },
      {
        name: "Mrs. Anshu Kumar",
        role: "Partners & International Business",
        image: "/about/Anshu_kumar.png"
      }
    ]
  }
};
